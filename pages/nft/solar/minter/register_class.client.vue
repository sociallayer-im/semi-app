<template>
    <div class="flex flex-col container-size rounded-xl bg-[var(--ui-bg)] shadow-lg p-4 "
        ...
        <div class="mx-aut p-3">
            <div class="text-2xl font-bold text-center mb-4">Register Class</div>
            <UForm :state="mintParams" @submit="handleMint" class="w-full flex flex-col gap-4">
                <UFormField name="registryAddress" label="Registry Address">
                    <UInput variant="subtle" size="md" class="w-full" v-model="mintParams.registryAddress"
                        placeholder="Enter contract address" />
                </UFormField>
                <UFormField name="classAddress" label="Class Address">
                    <UInput variant="subtle" size="md" class="w-full" v-model="mintParams.classAddress"
                        placeholder="Enter contract address" />
                </UFormField>
                <UFormField name="profileId" label="Profile ID">
                    <UInput variant="subtle" size="md" class="w-full" v-model="mintParams.profileId"
                        placeholder="Enter profile ID" type="number" />
                </UFormField>
                <UFormField name="chainId" label="Chain ID">
                    <USelect variant="subtle" size="md" class="w-full" v-model="mintParams.chainId"
...
import { badgeMinter, createClientConfig, chains } from 'solar-contract-sdk'
...
const handleMint = async () => {
    ...
    try {
        ...
        const txHash = await badgeMinter.registerClass({
            params: {
                profileId: BigInt(mintParams.value.profileId),
                classId: BigInt(mintParams.value.classId),
                contractAddr: mintParams.value.classAddress as `0x${string}`
            },
            contractAddress: mintParams.value.registryAddress as `0x${string}`,
            account: account,
            clientConfig
        })
        console.log(txHash)
