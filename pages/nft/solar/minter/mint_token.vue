    ...
    <div class="flex flex-col container-size rounded-xl bg-[var(--ui-bg)] shadow-lg p-4 "
        ...
        <div class="mx-aut p-3">
            ...
            <UForm :state="mintParams" @submit="handleMint" class="w-full flex flex-col gap-4">
                ...
                <UFormField name="to" label="To Address">
                    <UInput variant="subtle" size="md" class="w-full" v-model="mintParams.to"
                        placeholder="Enter contract address" />
                </UFormField>
                <UFormField name="tokenId" label="Token ID">
                    <UInput variant="subtle" size="md" class="w-full" v-model="mintParams.tokenId"
                        placeholder="Enter token ID" type="number" />
                </UFormField>
                <UFormField name="classId" label="Class ID">
                    <UInput variant="subtle" size="md" class="w-full" v-model="mintParams.classId"
                        placeholder="Enter class ID" type="number" />
                </UFormField>
...
const handleMint = async () => {
    ...
    try {
        ...
        const txHash = await badgeMinter.mintToken({
            params: {
                to: mintParams.value.to as `0x${string}`,
                tokenId: BigInt(mintParams.value.tokenId),
