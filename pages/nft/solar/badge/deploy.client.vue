        ...
        <div class="mx-aut p-3">
            ...
            <UForm :state="deployParams" @submit="handleDeploy" class="w-full flex flex-col gap-4">
                ...
                <UFormField name="name" label="Name">
                    <UInput variant="subtle" size="md" class="w-full" v-model="deployParams.name" />
                </UFormField>
                <UFormField name="symbol" label="Symbol">
                    <UInput variant="subtle" size="md" class="w-full" v-model="deployParams.symbol" />
                </UFormField>
...
const handleDeploy = async () => {
    if (!deployParams.value.name || !deployParams.value.symbol || !deployParams.value.registryAddress) {
        toast.add({
            title: 'Deploy failed',
            color: 'error',
    ...
    try {
        ...
        const receipt = await deploy.deployBadge({
            params: {
                name: deployParams.value.name,
                symbol: deployParams.value.symbol,
