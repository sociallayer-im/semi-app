  ...
  <div class="flex flex-col container-size rounded-xl bg-[var(--ui-bg)] shadow-lg p-4">
    ...
    <div class="flex flex-col items-center justify-center h-full gap-4 py-8 w-[80%] mx-auto">
      <h1 class="text-2xl font-bold mb-4">
        {{ i18nStore.text["Choose Wallet Initialization Method"] }}
      </h1>
      <div class="flex flex-col gap-6 w-full">
        <NuxtLink
          href="/paymentcode"
          class="hover:border-primary h-40 flex flex-1 flex-row gap-4 items-center border-[var(--ui-border)] border-2 rounded-xl p-4"
        >
          ...
          <div class="flex flex-col">
            ...
            <span class="text-sm text-gray-500">{{
              i18nStore.text["We will create a new wallet for you"]
            }}</span>
          </div>
        </NuxtLink>
        <NuxtLink
          href="/import"
          class="hover:border-primary flex flex-1 flex-row gap-4 items-center border-[var(--ui-border)] border-2 rounded-xl p-4"
        >
          <UIcon name="ci-arrow-circle-down" size="46" class="text-blue-500" />
          <div class="flex flex-col">
            <span class="text-lg font-bold">{{ i18nStore.text["Import"] }}</span>
            <span class="text-sm text-gray-500">{{
              i18nStore.text["Import an existing wallet"]
            }}</span>
          </div>
        </NuxtLink>
      </div>
