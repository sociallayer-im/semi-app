  ...
  >
    ...
    <UTabs :items="tabs" class="w-full overflow-auto" v-else>
      <template #send="{ item }">
        ...
        <div
          class="flex items-center gap-2 justify-between hover:bg-muted p-2 rounded-lg cursor-pointer"
          v-for="(action, index) in sendActions"
          :key="index"
          @click="toExplorer(action.txHex)"
        >
          <div class="flex flex-row gap-2">
            <div class="flex flex-col">
              ...
              <div
                class="text-gray-400 text-sm flex flex-row gap-1 items-center"
      ...
      <template #receive="{ item }">
        ...
        <div
          class="flex items-center gap-2 justify-between hover:bg-muted p-2 rounded-lg cursor-pointer"
          v-for="(action, index) in receiveActions"
          :key="index"
          @click="toExplorer(action.txHex)"
        >
          <div class="flex flex-row gap-2">
            <div class="flex flex-col">
              <div class="font-medium">
                <span class="text-green-400 text-sm">{{ i18n.text["FROM"] }}: </span
