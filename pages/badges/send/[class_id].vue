<template>
    <div
        ...
        <div class="flex-1 overflow-y-auto">
            <div v-if="step === 1" class="max-w-2xl mx-auto space-y-8">
                <UForm :state="formState" @submit="handleNext" class="space-y-4">
                    ...
                    <UFormField name="receiver_addresses" :label="i18n.text['Receiver Addresses']">
                        <div class="flex flex-col gap-3">
                            <div v-for="(address, index) in formState.receiver_addresses" :key="`receiver-${index}`"
                                class="flex items-start gap-2">
                                <UInput variant="subtle" size="md" v-model="formState.receiver_addresses[index]"
                                    :placeholder="i18n.text['Please enter receiver address']"
                                    @blur="validateField('receiver_addresses')" class="flex-1" />
                                <UButton v-if="formState.receiver_addresses.length > 1" color="neutral" variant="ghost"
                                    icon="i-heroicons-trash" :aria-label="i18n.text['Remove receiver']"
                                    type="button" @click="removeReceiver(index)" />
                            </div>
                            <div class="flex items-center gap-3">
                                <UButton color="primary" variant="soft" icon="i-heroicons-plus-circle" type="button"
                                    @click="addReceiver">
                                    {{ i18n.text['Add Receiver'] }}
                                </UButton>
                            </div>
                            <span v-if="errors.receiver_addresses"
                            class="text-sm text-error-500">{{ errors.receiver_addresses }}</span>
                        </div>
                    </UFormField>
            ...
            <div v-if="step === 3" class="max-w-2xl mx-auto flex flex-col gap-4 justify-center items-center">
                <UIcon name="ix:certificate-success" class="text-[90px] text-green-500" />
                <div class="text-2xl font-bold">{{ i18n.text['Send badges successful'] }}</div>
                <div>{{i18n.text['After the user accepts, they will receive a badge.']}}</div>
                <div class="flex flex-row flex-wrap justify-center gap-4">
                    <UButton type="submit" color="primary"  size="lg" class="w-full min-w-[160px] justify-center"
                        :disabled="isSubmitting" @click="router.push('/badges')">
                        {{ i18n.text['Back to My Badges'] }}
                    </UButton>
                    <UButton type="submit" color="neutral" size="lg" class="w-full min-w-[160px] justify-center"
                        :disabled="isSubmitting" :loading="isSubmitting" @click="step = 1">
...
const validateField = (field: keyof FormState) => {
    ...
    switch (field) {
        case 'image_url':
            errors.image_url = (value as string).trim() ? '' : t('Please upload badge image', 'Please upload badge image')
            break
        case 'name':
            errors.name = (value as string).trim() ? '' : t('Please enter badge name', 'Please enter badge name')
            break
        case 'description':
            errors.description = (value as string).trim() ? '' : t('Please enter badge description', 'Please enter badge description')
            break
        case 'receiver_addresses': {
            const addresses = (value as string[]).map((address) => address.trim()).filter(Boolean)
            if (!addresses.length) {
                errors.receiver_addresses = t('Please enter at least one receiver address', 'Please enter at least one receiver address')
                break
            }
